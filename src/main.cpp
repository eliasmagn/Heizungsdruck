...
PressureReading gLastReading; PressureState gLastState=PressureState::UNKNOWN; uint32_t gLastSampleMs=0; uint32_t gLastMinimalLogMs=0; bool gDebugVerbose=false;
DisplayState gDisplayState;
#if TARGET_ESP32
portMUX_TYPE gDisplayMux = portMUX_INITIALIZER_UNLOCKED;
#endif

String normalizedDeviceId(const String &in){String out;for(size_t i=0;i<in.length();++i){char c=static_cast<char>(tolower(in[i]));if((c>='a'&&c<='z')||(c>='0'&&c<='9')||c=='-'||c=='_') out+=c;}if(out.length()==0) out="kreis1";return out;}
void ensureDeviceIdentity(AppConfig &cfg){cfg.deviceId=normalizedDeviceId(cfg.deviceId.c_str()).c_str();const String base=String("heizungsdruck-")+cfg.deviceId.c_str();if(cfg.network.hostname.empty()||cfg.network.hostname=="heizungsdruck") cfg.network.hostname=base.c_str();if(cfg.mqtt.clientId.empty()||cfg.mqtt.clientId=="heizungsdruck") cfg.mqtt.clientId=base.c_str();if(cfg.mqtt.topicBase.empty()||cfg.mqtt.topicBase=="heizungsdruck") cfg.mqtt.topicBase=(String("heizungsdruck/")+cfg.deviceId.c_str()).c_str();}

#if TARGET_ESP32
void displayTask(void *){DisplayState local;for(;;){taskENTER_CRITICAL(&gDisplayMux);local=gDisplayState;taskEXIT_CRITICAL(&gDisplayMux);gDisplay.update(local);vTaskDelay(pdMS_TO_TICKS(500));}}
#endif
...
bool bridgeDebugEnabled(){
  ...
  return false;
#else
  pinMode(DEBUG_BRIDGE_OUT_PIN, OUTPUT); digitalWrite(DEBUG_BRIDGE_OUT_PIN, LOW); pinMode(DEBUG_BRIDGE_IN_PIN, INPUT_PULLUP); delay(2); int lowReads=0; for(int i=0;i<5;++i){ if(digitalRead(DEBUG_BRIDGE_IN_PIN)==LOW) lowReads++; delay(1);} return lowReads>=4;
#endif
}
...
bool saveCfg(const AppConfig &cfg){AppConfig normalized=cfg;ensureDeviceIdentity(normalized);if(!gStore.save(normalized)) return false;gConfig=normalized; if(gSensor) gSensor->updateConfig(gConfig); if(gStateMachine) gStateMachine->updateConfig(gConfig); gMqtt.begin(gConfig); gAlarm.updateConfig(gConfig); if(gConfig.wireguard.enabled) gWireGuard.enable(gConfig.wireguard); else gWireGuard.disable(); return true;}
...
const uint8_t protocol=only11b?WIFI_PROTOCOL_11B:(WIFI_PROTOCOL_11B|WIFI_PROTOCOL_11G|WIFI_PROTOCOL_11N); const esp_err_t r=esp_wifi_set_protocol(WIFI_IF_STA, protocol); return r==ESP_OK;
#else
(void)only11b; return false;
#endif
}
wifi_power_t txPowerFromDbm(float dbm){ if(dbm<=2.0f) return WIFI_POWER_2dBm; if(dbm<=5.0f) return WIFI_POWER_5dBm; if(dbm<=7.0f) return WIFI_POWER_7dBm; if(dbm<=8.5f) return WIFI_POWER_8_5dBm; if(dbm<=11.0f) return WIFI_POWER_11dBm; if(dbm<=13.0f) return WIFI_POWER_13dBm; if(dbm<=15.0f) return WIFI_POWER_15dBm; if(dbm<=17.0f) return WIFI_POWER_17dBm; if(dbm<=18.5f) return WIFI_POWER_18_5dBm; return WIFI_POWER_19_5dBm; }
#endif

void connectWifi(){ WiFi.mode(WIFI_STA); WiFi.setHostname(gConfig.network.hostname.c_str());
#if TARGET_ESP32
